    ...
    for file in matlab_annotations:
        ...
        with Image.open(images_dir + filename + ".jpg") as pil_image:
            ...
            for box in boxes.T:
                ...
                maxX = min(max(aXY[0], bXY[0], cXY[0], dXY[0]), width)
                minX = max(min(aXY[0], bXY[0], cXY[0], dXY[0]), 1)
                maxY = min(max(aXY[1], bXY[1], cXY[1], dXY[1]), height)
                minY = max(min(aXY[1], bXY[1], cXY[1], dXY[1]), 1)
                ...
                abs_intersect_x = (maxX + minX) / width / 2
                abs_intersect_y = (maxY + minY) / height / 2
